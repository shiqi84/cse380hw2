...
export default class Homework1_Scene extends Scene {
	...
	*/
	...
	loadScene(){
		...
		this.load.spritesheet("player", "hw2_assets/spritesheets/Shiqi_Wan_spaceship.json");
	...
	spawnAsteroid(): void {
		...
		asteroid.setColor(Color.RED);
			...
			asteroid.setColor(Color.GREEN);
			}
		else if(random<48){
			asteroid.setColor(Color.BLUE);
				}
		else if(random<64){
			asteroid.setColor(Color.WHITE);
					}
		else if(random<80){
			asteroid.setColor(Color.YELLOW);
						}
		else{
	...
	handleScreenWrap(node: GameNode, viewportCenter: Vec2, paddedViewportSize: Vec2): void {
		// Your code goes here:
		if(node.position.x>=paddedViewportSize.x){
			node.positionX=0;
		}
		else if(node.position.x<0){
			node.positionX=paddedViewportSize.x;
		};
		if(node.position.y>=paddedViewportSize.y){
			node.positionY=0;
		}
	...
	static checkAABBtoCircleCollision(aabb: AABB, circle: Circle): boolean {
		// Your code goes here
		 ...
		 const aabbHeigt=(aabb.topRight.x-aabb.topLeft.x)*(aabb.topRight.x-aabb.topLeft.x)
		 const aabbWidth=(aabb.topLeft.y-aabb.bottomLeft.y)*(aabb.topLeft.y-aabb.bottomLeft.y)
		 const halfDia=Math.pow((aabbHeigt+aabbWidth),0.5)/2
		 const x=(circle.x-aabb.center.x)*(circle.x-aabb.center.x)
		 const y=(circle.y-aabb.center.y)*(circle.y-aabb.center.y)
		 const distance=Math.pow((x+y),0.5)

		 if(halfDia+r>=distance){
			return true
		 }
		 else{
			 return false
